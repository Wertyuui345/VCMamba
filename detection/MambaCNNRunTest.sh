...
cd /work/09816/wertyuui345/ls6/VCMamba/VCMamba/detection;
...
then
    ...
    NCCL_DEBUG=INFO RANK=0 python -m torch.distributed.launch --nproc_per_node 1 --nnodes 1 --master_addr=$MASTER_ADDR --node_rank 0 --master_port=3456 --use_env main.py configs/mask_rcnn_mcnn_b_fpn_1x_coco.py --model VCMamba_EfficientFormer_B --work-dir detection_results/MambaCNN_B/ --launcher pytorch > detection_results/MambaCNN_B_run_det.txt
# elif [ $(scontrol show hostnames | sed -n '2p') = $LOCAL_ADDR ]
# then
#     echo Launching master @ $LOCAL_ADDR...
#     RANK=3 python -m torch.distributed.launch --nproc_per_node 3 --nnodes 3 --master_addr=$MASTER_ADDR --node_rank 1 --master_port=3456 --use_env train.py configs/sem_fpn/fpn_mcnn_b_ade20k_40k.py --resume-from semantic_results/MambaCNN_B/BEST_TEST_MambaCNN_B_4709.pth --model VCMamba_EfficientFormer_B --work-dir semantic_results/MambaCNN_B/ --launcher pytorch > semantic_results/MambaCNN_B_run_semantic.txt
# elif [ $(scontrol show hostnames | sed -n '3p') = $LOCAL_ADDR ]
# then
#     echo Launching master @ $LOCAL_ADDR...
#     RANK=6 python -m torch.distributed.launch --nproc_per_node 2 --nnodes 3  --master_addr=$MASTER_ADDR --node_rank 2 --master_port=3456 --use_env train.py configs/sem_fpn/fpn_mcnn_b_ade20k_40k.py --resume-from semantic_results/MambaCNN_B/BEST_TEST_MambaCNN_B_4709.pth --model VCMamba_EfficientFormer_B --work-dir semantic_results/MambaCNN_B/ --launcher pytorch > semantic_results/MambaCNN_B_run_semantic.txt
