...
class Embedding(nn.Module):
    def __init__(self, patch_size=3, stride=2, padding=1,
                 ...
                 light=False, asub=False, resolution=None, act_layer=nn.ReLU, attn_block=None):
        super().__init__()
        self.light = light
        self.asub = asub
...
def eformer_block(dim, index, layers,
    ...
    for block_idx in range(layers[index]):
        ...
        if index >= 2 and block_idx > layers[index] - 1 - vit_num:
            ...
            #     stride=stride,
            # ))
            blocks.append(models_mamba.create_block(
                    d_model = dim,
                    ssm_cfg=ssm_cfg,
...
# @register_model
# def VCMamba_Small(pretrained=False, **kwargs):
#     model = EfficientFormerV2(
#         layers=EfficientFormer_depth['S0'],
#         embed_dims=EfficientFormer_width['S0'],
#         downsamples=[True, True, True, True],
